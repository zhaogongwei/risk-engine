...
const EditableFormRow = Form.create()(EditableRow);
  ...
  export default class EditableCell extends PureComponent {
    ...
    getInput = () => {
      ...
      }else if(this.props.type === 'input' || this.props.record['kind']==='num'){
        return <Input
          ref={node => (this.input = node)}
          onPressEnter={this.save}
          onChange={(e) => this.changeHandler(e.target.value, this.props.record, this.props.dataIndex)}
        />;
      }else if(this.props.record['kind']==='str'){
        if(this.props.record['isenum']){
          return <Select
            onPressEnter={this.save}
            onChange={(e) => this.changeHandler(e, this.props.record, this.props.dataIndex)}
          >
            {
              this.props.record['enum']&&this.props.record['enum'].map((item,index)=>{
                return (
                  <Option value={item.id} key={index}>{item.name}</Option>
                )
              })
                ...
                return (
                  <FormItem style={{ margin: 0 }}>
                    {getFieldDecorator(`dataIndex${Math.random()}`, {
                      initialValue: record['kind']==='date'&&dataIndex==='val'?moment(record[dataIndex]?record[dataIndex]:new Date(), dateFormat):record[dataIndex],
                    })(this.getInput())}
                  </FormItem>
                );
              }}
